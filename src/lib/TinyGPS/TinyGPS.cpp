...
void TinyGPS::get_position(long *latitude, long *longitude, unsigned long *fix)
{
...
void TinyGPS::get_datetime(unsigned long *date, unsigned long *time, unsigned long *fix)
{
  if (date) *date = _date;
  if (time) *time = _time;
...
void TinyGPS::f_get_position(float *latitude, float *longitude, unsigned long *fix)
{
  long lat, lon;
  get_position2(&lat, &lon, fix);
...
void TinyGPS::sendCommand(const char *str)
{
  
}

void TinyGPS::suspend();
void TinyGPS::wakeup();
